...
float F310::GetX(StickType stick)
{
	...
	{
	case kRightStick:
		result = GetRawAxis(kRightXAxisNum);
		break;
	case kLeftStick:
		result = GetRawAxis(kLeftXAxisNum);
		break;
	}
...
float F310::GetY(StickType stick)
{
	...
	{
	case kRightStick:
		result = GetRawAxis(kRightYAxisNum);
		break;
	case kLeftStick:
		result = GetRawAxis(kLeftYAxisNum);
		break;
	}
...
float F310::GetAxis(AxisType axis)
{
	switch(axis)
	{
	    ...
	    default:
	    	printf("Joystick axis is out of range");
	    	return 0.0;
	}
}
